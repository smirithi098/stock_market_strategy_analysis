...
print(f"Total returns from the Bollinger bands with RSI strategy = {round(total_return_2, 2)}%")
...
buy_sell_points_3[:]['position'] = np.where(buy_sell_points_3['buy_signal'] == 1, 'buy', 'sell')

buy_sell_points_3 = strat.filter_buy_sell_points(buy_sell_points_3)

buy_sell_data_3 = strat.identify_buy_sell_points(buy_sell_points_3)
...
ax1.plot(data_subset_3.close, color='#4A55A2', label='Close price', linewidth=1.3)
...
print(f"Total returns from the MACD with 200-day EMA strategy = {round(total_return_3, 2)}%")
...
print(f"Total returns from the MACD with RSI strategy = {round(total_return_4, 2)}%")
